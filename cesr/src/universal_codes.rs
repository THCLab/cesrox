...
use nom::{bytes::complete::take, character::complete::anychar, error::{make_error, ErrorKind}, IResult};

use crate::{conversion::{adjust_with_num, b64_to_num, num_to_b64}, derivation_code::DerivationCode, error::Error, primitives::codes::PrimitiveCode};
...
pub enum UniversalGroupCode {
	...
	Special { code: SpecialCountCode, quadlets: u16 },
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		match code {
			...
			x if x.is_alphabetic() => {
				...
				Ok(Self::Special { code: special_code, quadlets: length.into() })
			},
...
impl ToString for GenusCountCode {
	fn to_string(&self) -> String {
		match self {
			GenusCountCode::Keri { minor, major } => format!("_AAA{}{}", num_to_b64(*major), adjust_with_num(*minor, 2)),
		}
	}
}
...
impl DerivationCode for UniversalGroupCode {
	fn hard_size(&self) -> usize {
		match self {
			...
			UniversalGroupCode::Special { code, quadlets: _ } => {
				match code {
	...
	fn soft_size(&self) -> usize {
		match self {
			...
			UniversalGroupCode::Special { code: _, quadlets: _ } => 2,
		}
	}
	...
	fn to_str(&self) -> String {
		match self {
			...
			UniversalGroupCode::Special { code, quadlets } => format!("{}{}", code.to_string(), adjust_with_num(quadlets.clone(), 2)),
		}	
	}
}
